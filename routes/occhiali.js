...
router.post('/',async(req,res)=>{
    ...
    res.send('Occhiale aggiunto con successo<br> Premi <a href="/authHomeAdmin">qui</a> per la homepage')}
...
router.delete('/:id',async(req,res)=>{
    ...
    res.json({message:'utente eliminato con successo<br><a href="/authHomeAdmin>Torna alla home</a>'})}
    catch (error) {res.status(404).json({message:error.message})}})

router.post('/:id',async(req,res)=>{
    const {id} = req.params
    const newOcchiale={...req.body}
    if (newOcchiale.password!='ma') {return res.send('password errata<br><a href="/authHomeAdmin">Torna inidietro</a>')}
    if (!mongoose.Types.ObjectId.isValid(id))
    ...
    try { 
        ...
        res.send(`occhiale con id ${occhiale._id} modificato con successo<br><a href="/authHomeAdmin">Torna alla home</a>`)}
    catch (error) {res.status(404).json({message:error.message})}})
