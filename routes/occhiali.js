...
router.delete('/:id',async(req,res)=>{
    ...
    res.json({message:'utente eliminato con successo<br><a href="../catalogo.html>Torna alla home</a>'})}
    catch (error) {res.status(404).json({message:error.message})}})
...
router.post('/:id',async(req,res)=>{
    ...
    try { 
        ...
        res.send(`occhiale con id ${occhiale._id} modificato con successo<br><a href="../catalogo.html">Torna alla home</a>`)}
    catch (error) {res.status(404).json({message:error.message})}})
